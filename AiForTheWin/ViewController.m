...
@synthesize machineVoice, personVoice,negative,positive, nouns, personalThoughts, familyThoughts,workThoughts, synthesizer, voice, UserQUESTIONS, positveVerbs, negativeVerbs, subjects, PersonName, arrayOfSports;
...
- (void)viewDidLoad {
    ...
    arrayOfSports = [[NSMutableArray alloc]initWithObjects:@"dog",@"cat",@"fish",@"rat",@"mouse",@"jelly fish",@"ferit",@"crab",@"hamster",@"spider", @"turtle",@"snake",@"slug",@"worm",@"ant",@"whale",@"zebra",@"toad",@"tiger",@"lion", @"leopard",@"panda",@"shark",@"gorilla",@"chimp",@"monkey",@"sloth",@"elephant",@"camel",@"horse", @"giraffe",@"ostrage",@"wolf",@"rhino",@"hippo",@"star fish",@"dinosaurs",@"lima",@"koala bear",@"bat", @"raccoon",@"butterfly",@"ladybug",@"possum",@"bee",@"wasp cat",@"blue jay",@"pigeon",@"wood pecker",@"emu", @"frog",@"polar bear",@"seagul",@"blob fish",@"crocodile",@"worm",@"turkey",@"chicken",@"duck",@"jaguar", @"panther",@"coyote",@"penguin",@"ram",@"elk",@"sheep",@"gerbil",@"bull",@"rooster",@"cheetah", @"owl",@"ape",@"goose",@"parrot",@"cobra",@"seal",@"kangaroo",@"hyene",@"catfush",@"chipmunks", @"whale",@"alligator",@"catfish",@"antelope",@"toad",@"lama",@"hyena",@"chimpanzee",@"black bear",@"brown bear", @"trout",@"blad eagle",@"eagle",@"cougar",@"swordfish",@"poodle",@"swan",@"raven",@"lobster",@"dolphin", nil];
    ...
    UserQUESTIONS = [[NSMutableArray alloc] initWithObjects: @"What are you?", @"where do you come from", @"where did you come from", @"who are you", @"what is your name", @"where are you from",
                     nil];
    ...
    subjects=[[NSMutableArray alloc] initWithObjects: @"he", @"it", @"she", @"i",
              nil];
    ...
    ////if sentence contains to or more subjects, let the machine only call out the verb
    
}

-(void)viewDidAppear:(BOOL)animated {
    
    if(personVoice.isEditing){
        
        self.titleLabel.hidden = YES;
        personVoice.bounds = CGRectMake(self.titleLabel.frame.origin.x,self.titleLabel.frame.origin.y, personVoice.frame.size.width , personVoice.frame.size.height);
...
-(void) animals {
    ...
    for (int i = 0; i < [animals count]; i++) {
        NSString *animalString = [animals objectAtIndex:i];
